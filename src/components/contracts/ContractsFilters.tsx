...
export const ContractsFilters: React.FC<FiltersProps> = ({
  filters,
  onFiltersChange
}) => {
  const statusOptions = [
    { value: 'all', label: 'Todos os Status', icon: Eye, color: 'text-gray-600' },
    { value: 'draft', label: 'Rascunho', icon: FileSignature, color: 'text-gray-600' },
    { value: 'pending_signature', label: 'Pendente Assinatura', icon: Clock, color: 'text-yellow-600' },
    { value: 'signed', label: 'Assinado', icon: CheckCircle, color: 'text-green-600' },
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateFilters('signatureProvider', e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent"
          >
      ...
      {/* Filtros por Provedor de Assinatura */}
      <div className="mt-3">
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            { value: 'docusign', label: 'DocuSign', icon: Shield, color: 'text-blue-600' },
            { value: 'clicksign', label: 'ClickSign', icon: PenTool, color: 'text-green-600' },
            { value: 'internal', label: 'Interno', icon: FileSignature, color: 'text-purple-600' }
          ].map((provider) => (
            <motion.button
              key={provider.value}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => updateFilters('signatureProvider', provider.value)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filters.signatureProvider === provider.value
                  ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300'
                  : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
              }`}
            >
